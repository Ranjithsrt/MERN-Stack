    ...
    <div className="bg-white rounded-xl overflow-hidden cursor-pointer group hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col">
      <div className="relative h-64 overflow-hidden bg-slate-50">
        ...
        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button className="bg-white p-2.5 rounded-full shadow-lg text-slate-800 hover:text-indigo-600 hover:bg-slate-50 transition-colors" title="Add to Cart" onClick={() => dispatch({ type: "ADD_TO_CART", payload: product })}>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-50">
          ...
          <button className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 hover:underline transition-all" onClick={() => dispatch({ type: "ADD_TO_CART", payload: product })}>
            Add to Cart
          </button>
        </div>
      </div>
