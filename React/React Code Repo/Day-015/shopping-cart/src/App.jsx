      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div className="flex items-center justify-between max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 h-20">
          ...
          <div className="p-2 relative hover:bg-slate-100 rounded-full transition-colors cursor-pointer">
            ...
            {totalItems > 0 && <span className="absolute top-0 right-1 bg-indigo-600 text-white text-[12px] border-white shadow-sm ring-1 ring-indigo-600 h-5 w-5 font-bold flex items-center justify-center rounded-full">{totalItems}</span>}
          </div>
        </div>
      </header>
            ...
            {state.cart.length > 0 ? (
              <>
                ...
                <div className="space-y-5">
                  ...
                  <CartSummary total={state.total} cartItemCount={totalItems} dispatch={dispatch} />
                </div>
              </>
            ) : (
