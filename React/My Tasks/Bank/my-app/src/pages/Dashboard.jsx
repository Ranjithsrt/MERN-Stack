 ...
 const StatCard = ({
   ...
   <div
     className={`p-8 rounded-2xl shadow-sm relative overflow-hidden text-white ${gradient}`}
   >
     <div className="relative z-10">
       <div className="flex justify-between items-start mb-6">
         <p className="text-sm font-semibold opacity-90 tracking-tight">
           {title}
         </p>
         <div className="px-3 py-1 bg-white/20 rounded-md text-[10px] font-bold backdrop-blur-md uppercase">
           {percentage ? percentage : "Active"}
         </div>
       </div>
       <h3 className="text-3xl md:text-4xl font-bold tracking-tighter mb-4 flex items-baseline gap-2">
         <span className="text-base opacity-60 font-medium">{currency}</span>{" "}
         {amount}
       </h3>
       {secondaryLabel && (
     ...
     <div className="space-y-8">
       <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
         <StatCard
           ...
           amount={formatAmount(account.balance)}
           currency="₹"
         ...
         <StatCard
           ...
           amount="6.12%"
           currency=""
           gradient="bg-gradient-to-br from-[#f8a5c2] to-[#ff9f91]"
         ...
         <div className="p-8 rounded-2xl shadow-sm bg-linear-to-br from-[#c1f4c1] to-[#98d898] relative overflow-hidden text-[#225024] flex flex-col justify-between min-h-[180px]">
           <div className="relative z-10">
             <div className="flex justify-between items-center mb-6">
               ...
               <p className="font-mono text-[10px] font-bold opacity-30 uppercase">
                 Sovereign Card
               </p>
             </div>
             <p className="text-xl font-mono font-bold tracking-[0.2em] mb-4 text-[#225024]/80 text-center">
               3829 4820 4629 5025
             </p>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
           ...
           <div className="flex flex-col md:flex-row justify-between items-center gap-6">
             <div className="flex gap-12">
               ...
               <div className="border-l border-slate-50 pl-12">
                 ...
                 <div className="flex items-center gap-2">
                   <p className="text-2xl font-bold text-emerald-500 tracking-tighter">
                     ₹5.8k
                   </p>
                 </div>
               </div>
             ...
             <div className="flex items-end gap-3 h-20">
               {[...Array(12)].map((_, i) => (
                 <div
                   key={i}
                   className="w-2.5 rounded-sm bg-slate-50 h-full relative overflow-hidden"
                 >
                   <div
                     className={`absolute bottom-0 w-full rounded-sm transition-all duration-1000 ${
                       i % 2 === 0 ? "bg-orange-400" : "bg-indigo-600"
                     }`}
         ...
         <div className="lg:col-span-9 space-y-5">
           <div className="flex items-center justify-between px-2">
             <h4 className="text-lg font-bold text-slate-800 tracking-tight uppercase">
               Activity Ledger
             </h4>
             ...
             {recentTransactions.length > 0 ? (
               <div className="divide-y divide-slate-50">
                 {recentTransactions.map((tx) => (
                   <div
                     key={tx.id}
                     className="p-4 px-6 flex items-center justify-between hover:bg-slate-50/50 transition-all group"
                   >
                     <div className="flex items-center space-x-6">
                       <div
                         className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all bg-[#f8fafc] border border-slate-100`}
                       >
                         {tx.type === "DEPOSIT" ||
                     ...
                     <p
                       className={`text-lg font-bold tabular-nums tracking-tighter ${
         ...
         <div className="lg:col-span-3 space-y-5">
           <h4 className="text-lg font-bold text-slate-800 tracking-tight uppercase px-2">
             Actions
           </h4>
           <div className="bg-white rounded-xl p-6 border border-slate-100 space-y-3 shadow-sm">
             <Link
               to="/deposit"
               className="flex items-center justify-between p-3.5 bg-indigo-50/50 rounded-lg hover:bg-indigo-50 transition-colors group border border-transparent hover:border-indigo-100"
             >
             ...
             <Link
               to="/withdraw"
               className="flex items-center justify-between p-3.5 bg-orange-50/50 rounded-lg hover:bg-orange-50 transition-colors group border border-transparent hover:border-orange-100"
             >
               <div className="flex items-center gap-3">
